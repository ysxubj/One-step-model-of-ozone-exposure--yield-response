...
DF.class.long %>% filter(PDFnumber=="2006-90")
...
Totaldata %>% filter(Area =="Europe")->Totaldata_area
...
ggsave(file.path(Outputfolder,'Europe_wheat_CI.png'),width = 20, height = 16,units = "cm",dpi=400)
...
ggsave(file.path(Outputfolder,paste0('Figure','/Europe_1000_AOT40.png')),width = 12, height = 8,units = "cm",dpi=400)
...
ggsave(file.path(Outputfolder,paste0('Figure','/Europe_1000_M12.png')),width = 12, height = 8,units = "cm",dpi=400)

pars[2]<-y
RY_result<-list(result,modelresult,pars,percentile_intervals)
export(RY_result,file.path(Outputfolder,"Europe_wheat_result_1000.xlsx"),overwrite=T)

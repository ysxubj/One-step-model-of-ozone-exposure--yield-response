...
Totaldata %>% filter(`country of experiment` %in%  c("China","India"))->Totaldata_area

DF.plot<-Model_output_fun(0.011,10,Totaldata_area)
...
#ggsave(file.path(Outputfolder,paste0('Figure','/Aisa_wheat_AOT40.png')),width = 12, height = 8,units = "cm",dpi=400)
...
#ggsave(file.path(Outputfolder,paste0('Figure','/Aisa_wheat_M12.png')),width = 12, height = 8,units = "cm",dpi=400)

pars[2]<-y
RY_result<-list(result,modelresult,pars,percentile_intervals)
export(RY_result,file.path(Outputfolder,"Aisa_wheat_result_Asia.xlsx"),overwrite=T)

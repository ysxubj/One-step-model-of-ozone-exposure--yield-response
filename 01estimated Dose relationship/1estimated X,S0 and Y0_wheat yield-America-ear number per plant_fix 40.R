...
DF.plot<-Model_output_fun(0.01,Totaldata_area)
...
#ggsave(file.path(Outputfolder,paste0('Figure','/N_America_ear number per plant_AOT40.png')),width = 12, height = 8,units = "cm",dpi=400)
...
#ggsave(file.path(Outputfolder,paste0('Figure','/N_America_ear number per plant_M12.png')),width = 12, height = 8,units = "cm",dpi=400)

RY_result<-list(result,modelresult,pars,percentile_intervals)
export(RY_result,file.path(Outputfolder,"N_America_wheat_result_estY_ear number per plant_fix40.xlsx"),overwrite=T)
...
ggplot()+
  ...
  geom_text(aes(x=5,y=0.1),label=paste0("S=",round(S,4)))+
  geom_text(aes(x=5,y=0),label=paste0("y=",round(40,4)))
#ggsave(file.path(Outputfolder,paste0('Figure','/N_America_withoutKohut_ear number per plant_AOT40.png')),width = 12, height = 8,units = "cm",dpi=400)
...
ggplot()+
  ...
  geom_line(data=result,aes(x=M12 ,y=RY_upper),color='red')+
  geom_line(data=result,aes(x=M12 ,y=RY_lower),color='blue')
#ggsave(file.path(Outputfolder,paste0('Figure','/N_America_withoutKohut_ear number per plant_M12.png')),width = 12, height = 8,units = "cm",dpi=400)

RY_result<-list(result,modelresult,pars,percentile_intervals)
export(RY_result,file.path(Outputfolder,"N_America_withoutKohut_wheat_result_estY_ear number per plant_fix40.xlsx"),overwrite=T)

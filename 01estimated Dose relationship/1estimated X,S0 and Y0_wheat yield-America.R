...
#filter data
DF %>%filter(Accept!=0) %>% select("EXP_ID","PDFnumber","Author","Area","species","cultivar/genotype","exp.Year","experimental facility","rooting","O3Trt","AOT40","O3 con","O3 moniter hour","AOT40 Days","N", "AOT40_Adjusted","yield","1000 grain weight","Total grain no.")->DF.filter

#filter
DF.filter  %>% pivot_longer(cols = 17:19)->DF.class.long
...
ggsave(file.path(Outputfolder,paste0('Figure','/N_America_wheat_AOT40.png')),width = 12, height = 8,units = "cm",dpi=400)
...
ggsave(file.path(Outputfolder,paste0('Figure','/N_America_wheat_M12.png')),width = 12, height = 8,units = "cm",dpi=400)



pars[2]<-y
RY_result<-list(result,modelresult,pars,percentile_intervals)
export(RY_result,file.path(Outputfolder,"N_America_wheat_result_estY.xlsx"),overwrite=T)
...
ggplot()+
  ...
  geom_text(aes(x=5,y=0.1),label=paste0("S=",round(S,4)))+
  geom_text(aes(x=5,y=0),label=paste0("y=",round(y,4)))
ggsave(file.path(Outputfolder,paste0('Figure','/N_America_withoutKohut_wheat_AOT40.png')),width = 12, height = 8,units = "cm",dpi=400)
...
ggplot()+
  ...
  geom_line(data=result,aes(x=M12 ,y=RY_upper),color='red')+
  geom_line(data=result,aes(x=M12 ,y=RY_lower),color='blue')
ggsave(file.path(Outputfolder,paste0('Figure','/N_America_withoutKohut_wheat_M12.png')),width = 12, height = 8,units = "cm",dpi=400)

pars[2]<-y
RY_result<-list(result,modelresult,pars,percentile_intervals)
export(RY_result,file.path(Outputfolder,"N_America_withoutKohut_wheat_result_estY.xlsx"),overwrite=T)
